...
Scenario
An e-commerce platform contains thousands of products. A customer selects a category, and the application 
needs to show only the products belonging to that category. Each product has a name, price, and category.

Task
Create a function named findProducts. The function should receive an array of products and a category, 
and return all products that belong to that category.
...
function findProducts(products:Product[],category:string):Product[]{
    ...
    // const matchCategory=products.filter((item)=>{
    //     if(item.category===category){
    //         return item;
    //     }
    // })

    const matchCategory:Product[]=products.filter((item:Product)=>item.category===category);
